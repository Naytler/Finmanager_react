  ...
  useEffect(() => {
    for (let i = 0; i < filterData.length; i++) {
      if (filterData[i].split('::')[2] === 'покупка продуктов') {
        setR01((prev) => prev + +filterData[i].split('::')[0].split(' '));
      }
      if (filterData[i].split('::')[2] === 'оплата счетов') {
        setR02((prev) => prev + +filterData[i].split('::')[0].split(' '));
      }
      if (filterData[i].split('::')[2] === 'покупка одежды') {
        setR03((prev) => prev + +filterData[i].split('::')[0].split(' '));
      }
      if (filterData[i].split('::')[2] === 'расходы на транспорт') {
        setR04((prev) => prev + +filterData[i].split('::')[0].split(' '));
      }
      if (filterData[i].split('::')[2] === 'развлечения') {
        setR05((prev) => prev + +filterData[i].split('::')[0].split(' '));
      }
      if (filterData[i].split('::')[2] === 'путешествия') {
        setR06((prev) => prev + +filterData[i].split('::')[0].split(' '));
      }
    }
  }, []);
